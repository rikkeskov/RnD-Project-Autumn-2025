def Transform(shrink):
    ...
    TempMergedSegments.sort(
        key=lambda segment: (segment.get_init_timestamp, segment.get_b, segment.get_a)
    )
    ...
    for i in range(len(TempMergedSegments) - 1):
        ...
        new_segments.append(
            [new_first_element, TempMergedSegments[i][1], TempMergedSegments[i][2]]
        )
    new_first_element_last = [TempMergedSegments[-1][0], shrink.lastTimeStamp]
    new_segments.append(
        [new_first_element_last, TempMergedSegments[-1][1], TempMergedSegments[-1][2]]
    )

    representatives = FindRepresentatives(
        new_segments, shrink.length
    )  ### format of new_segments = [[t1, t2], slope, b, score]
...
def FilterOutLier(segments):
    ...
    while i <= len(segments) - 1:
        seg = segments[i]
        if seg[0][-1] - seg[0][0] <= 3 and (
            seg[-1] not in occurency.keys()
        ):  # 包含元素较少，不超过3
            index.append(i)
            representatives.append(seg)
            pointsNum += countPoints(seg)
            occurency[seg[-1]] = {
                round(seg[-2] * 10000): (seg[0][-1] - seg[0][0], seg[-2], seg[-1])
            }
...
def DeTransform(segments, percentage=100):
    ...
    for i in range(len(segments)):
        ...
        indexs += timestamps
        points += [
            a_values[i] * (ts - init_timestamps[i][0]) + b_values[i]
