...
class SimPieceSegment:
    ...
    def round_to_least_decimal(self, aMin, aMax):
        ...
        decimal_places_aMin = len(str(aMin).split(".")[1].rstrip("0")) - len(
            str(aMin).split(".")[1].lstrip("0")
        )
        decimal_places_aMax = len(str(aMax).split(".")[1].rstrip("0")) - len(
            str(aMax).split(".")[1].lstrip("0")
        )
