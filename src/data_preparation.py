...
def ts_to_pd_dataframe(
...
) -> pd.DataFrame | tuple[pd.DataFrame, npt.NDArray[Any]]:
    ...
    return_separate_X_and_y: bool
        true if X and Y values should be returned as separate Data Frames (X)
        and a numpy array (y), false otherwise.
    ...
    -------
    DataFrame, ndarray
        If return_separate_X_and_y then a tuple containing a DataFrame
        and a numpy array containing the relevant time-series and corresponding class values.
    DataFrame
    ...
    num_dimensions = None
    is_first_case = True
    instance_list: list[list[pd.Series]] = []
    class_val_list: list[float] = []
    line_num = 0

    # ensure non-unbound variables
    timestamps = False
    target_labels = False
    ...
    with open(full_file_path_and_name, "r") as file: # pylint-disable: unspecified-encoding
        for line in file:
            # print(".", end='')
            ...
            if line:
                ...
                # Please note that even though metadata is stored in this function
                # it is not currently published externally
                if line.startswith("@problemname"):
                    # Check that the data has not started
                    if data_started:
                        raise ValueError("metadata must come before data")
                    # Check that the associated value is valid
                    tokens = line.split(" ")
                    token_len = len(tokens)

                    if token_len == 1:
                        raise ValueError("problemname tag requires an associated value")

                    problem_name = line[len("@problemname") + 1 :]
                    logging.info("problem name: %s", problem_name)
                    ...
                    if token_len != 2:
                        ...
                        raise ValueError("invalid timestamps value")
                    ...
                    if token_len != 2:
                        ...
                        raise ValueError("invalid univariate value")
                    logging.info("Is the data univariate? %s", str(univariate))
                    ...
                    if tokens[1] == "true":
                        ...
                        raise ValueError("invalid classLabel value")
                    ...
                    logging.info("Class labels: %s", class_label_list)
                    metadata_started = True
                    ...
                    if tokens[1] == "true":
                        ...
                        raise ValueError("invalid targetLabel value")
                ...
                # Check if this line contains the start of data
                    ...
                    if line != "@data":
                        raise ValueError("data tag should not have an associated value")
                    ...
                    if data_started and not metadata_started:
                        ...
                        has_data_tag = True
                        data_started = True
                # If the 'data tag has been found then metadata has been parsed
                # and data can be loaded
                    ...
                    if timestamps:
                        ...
                        timestamps_for_dimension: (
                            list[str | int | float] | pd.DatetimeIndex
                        ) = []
                        values_for_dimension: list[float] = []
                        ...
                        while char_num < line_len:
                            ...
                            # See if there is any more data to read in
                            # or if we should validate that read thus far
                            ...
                            if char_num < line_len:
                                ...
                                if line[char_num] == ":":
                                    ...
                                    if line[char_num] != "(" and target_labels:
                                        ...
                                        # Check if there is another value or dimension to process
                                        # after this tuple
                                        ...
                                        # Get the numeric value for the tuple by reading
                                        # from the end of the tuple data backwards to the last comma
                                        ...
                                        if (
                                            ...
                                            raise ValueError(
                                                ...
                                                + " on line "
                                                + str(line_num + 1)
                                                + " contains a tuple that has invalid timestamp '"
                                                + str(timestamp)
                                                + "'"
                                            )
                                        ...
                                        if (
                                            ...
                                            raise ValueError(
                                                ...
                                                + " on line "
                                                + str(line_num + 1)
                                                + " contains tuples where timestamp is inconsistent"
                                            )
                            ...
                            if not has_another_value and not has_another_dimension:
                                ...
                                if num_dimensions != this_line_num_dimensions:
                                    raise ValueError(
                                        "line "
                                        + str(line_num + 1)
                                        + " does not have the same number of dimensions"
                                        " as the previous line of data"
                                    )
                        ...
                        if has_another_dimension and target_labels:
                            raise ValueError(
                                "dimension "
                                + str(this_line_num_dimensions + 1)
                                + " on line "
                        ...
                        if has_another_dimension and not target_labels:
                            if len(instance_list) < (this_line_num_dimensions + 1):
                                instance_list.append([])
                        ...
                        # Check if we should have class values,
                        # and if so that they are contained in those listed in the metadata
                        ...
                        # See how many dimensions that the case whose
                        # data in represented in this line has
                        ...
                        if num_dimensions is None:
                            raise ValueError(
                                "Number of dimensions should not be "
                                "None at this point."
                            )
                        for dim in range(0, num_dimensions):
    ...
    if line_num:
        ...
        if (
            ...
            raise ValueError("file contained metadata but no data")
...
def pd_dataframe_to_csv(
    ...
    base_filepath = target_filepath.split(".")[1]
    print(base_filepath)
